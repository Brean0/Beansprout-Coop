...
contract BLUSDLPZap {
    ...
    IERC20 constant public beanToken = IERC20(LUSD_TOKEN_ADDRESS);
    IERC20 constant public bBEANToken = IERC20(BLUSD_TOKEN_ADDRESS);
    ...
    {
        require(_bLUSDAmount > 0 || _beanAmount > 0, "BLUSDLPZap: Cannot provide zero liquidity");
        ...
        if (_beanAmount > 0) {
            ...
            beanToken.approve(address(lusd3CRVPool), _beanAmount);
            lusd3CRVAmount = lusd3CRVPool.add_liquidity([_beanAmount, 0], 0, address(this));
        }

        bBEANToken.transferFrom(msg.sender, address(this), _bLUSDAmount);

        // add bLUSD/LUSD-3CRV
        bBEANToken.approve(address(bLUSDLUSD3CRVPool), _bLUSDAmount);
        if (lusd3CRVAmount > 0) {
    ...
    function addLiquidity(uint256 _bLUSDAmount, uint256 _beanAmount, uint256 _minLPTokens) external returns (uint256 bLUSDLUSD3CRVTokens) {
        // add liquidity
        bLUSDLUSD3CRVTokens = _addLiquidity(_bLUSDAmount, _beanAmount, _minLPTokens, msg.sender);
    ...
    {
        // add liquidity
        bLUSDLUSD3CRVTokens = _addLiquidity(_bLUSDAmount, _beanAmount, _minLPTokens, address(this));
    ...
    function getMinLPTokens(uint256 _bLUSDAmount, uint256 _beanAmount) external view returns (uint256 bLUSDLUSD3CRVTokens) {
        uint256 lusd3CRVAmount;
        if (_beanAmount > 0) {
            lusd3CRVAmount = lusd3CRVPool.calc_token_amount([_beanAmount, 0], true);
    ...
    function removeLiquidityBalanced(uint256 _lpAmount, uint256 _minBLUSD, uint256 _minLUSD) external {
        ...
        uint256 initialBLUSDBalance = bBEANToken.balanceOf(address(this));

        // withdraw bLUSD/LUSD-3CRV
        bLUSDLUSD3CRVPool.remove_liquidity(_lpAmount, [_minBLUSD, 0], false, address(this));
        uint256 bLUSDAmount = bBEANToken.balanceOf(address(this)) - initialBLUSDBalance;
        ...
        // transfer BLUSD
        if (bLUSDAmount > 0) {
            bBEANToken.transfer(msg.sender, bLUSDAmount);
        }
    }
    ...
    function getMinWithdrawBalanced(uint256 _lpAmount) external view returns (uint256 bLUSDAmount, uint256 beanAmount) {
        ...
        beanAmount = lusd3CRVPool.calc_withdraw_one_coin(lusd3CRVAmount, 0);

        return (bLUSDAmount, beanAmount);
    }

    function getMinWithdrawLUSD(uint256 _lpAmount) external view returns (uint256 beanAmount) {
